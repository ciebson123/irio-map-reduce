...
def test_big_files_multiple_workers(worker_ports: List[int], master_client: MasterStub, num_partitions: int):
    for worker_port in worker_ports:
        master_client.RegisterService(RegisterServiceMes(service_address="localhost", service_port=worker_port))

    response = master_client.MapReduce(
        MapReduceRequest(input_dir=TEST_INPUT_DIR.absolute().as_posix(), num_partitions=num_partitions),
    )

    assert read_mapreduce_outputs(Path(response.output_dir)) == count_words(TEST_INPUT_DIR)
